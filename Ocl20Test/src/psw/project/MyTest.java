...
import impl.ocl20.environment.EnvironmentFactory;
 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class MyTest extends TestPswProject {

	public static void main(String[] args) throws Exception {
		MyTest m = new MyTest();
		
//		m.testCreateNewProject();
//		m.testOpenProject();
//		m.testCompileFile();
//		m.testCompileProject();
//		m.testRemoveOclFile();
		Monitor mon = MonitorFactory.start("test loyalty project");
		m.testLoyaltyProject();
		mon.stop();
...
//		m.testBookProject();
	...
	public	void	testLoyaltyProject() {
		...
		try {		
			PswProjectSession	session = new PswProjectSession();
			Monitor m1 = MonitorFactory.start("test loyalty create Project");
			PswProject project = session.createProject(name, metamodel, xmiFile, oclFiles);
			m1.stop();
			
//			CoreClassifier bool = (CoreClassifier) project.getEnvironment().lookup("Integer");
//			assertNotNull(bool);
...
//			}
//			
			m1 = MonitorFactory.start("test loyalty compile Project");
			Set	errors = session.compileProject();
			m1.stop();
	...
	public	void	testBookProject() {
		...
		try {		
			PswProjectSession	session = new PswProjectSession();
			Monitor m1 = MonitorFactory.start("test book create Project");
			PswProject project = session.createProject(name, metamodel, xmiFile, oclFiles);
			m1.stop();
			Monitor m2 = MonitorFactory.start("test book compile Project");
			Set	errors = session.compileProject();
			m2.stop();
			System.out.println("\nerrors - " + name + ": ");
			for (Iterator iter = errors.iterator(); iter.hasNext();) {
			...
			for (int i = 10; i < 30; i++) {
				Monitor m3 = MonitorFactory.start("test book compile Project again - " + i);
				errors = session.compileProject();
				m3.stop();
		...
		System.out.println("Environment instances: " + EnvironmentFactory.n);
	}
